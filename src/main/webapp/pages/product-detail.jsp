    ...
    <section class="product-container" aria-labelledby="productTitle">
        ...
        <aside class="side-panel" data-aos="fade-left" aria-label="Thanh mua hàng">
            ...
            <div style="margin-top:20px">
                <form id="addCartForm" action="${pageContext.request.contextPath}/add-cart?id=${p.id}&q=1" method="get">
    ...
    (function () {
        ...
        function showToast(message, isSuccess = true) {
            if (toast && toastText) {
                ...
                if (icon) {
                    icon.className = isSuccess ? 'bi bi-check-circle-fill text-success me-2' : 'bi bi-x-circle-fill text-danger me-2';
                }
        ...
        if (addCartForm) {
            addCartForm.addEventListener('submit', function(e) {
                ...
                // Send AJAX request
                fetch(`${pageContext.request.contextPath}/add-cart?id=${id}&q=${qty}`, {
                    method: 'GET'
                })
                    .then(response => {
                        if (response.ok) {
                            showToast(`Đã thêm ${qty} sản phẩm vào giỏ hàng!`, true);
    ...
    (function () {
        ...
        if (writeBtn) {
            writeBtn.addEventListener('click', () => {
                ...
                if (name) {
                    const review = prompt('Đánh giá của bạn (tối đa 300 ký tự):');
                    if (review) {
                        alert(`Cảm ơn ${name} đã đánh giá sản phẩm!`);
                    }
                }
            });
        }
